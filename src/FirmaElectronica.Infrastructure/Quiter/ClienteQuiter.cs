...
public sealed class ClienteQuiter(HttpClient http, QuiterOptions opciones) : IQuiterClient
{
    ...
    public async Task ActualizarContactoClienteAsync(ContactoClienteQuiter contacto, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(opciones.ClientId) || string.IsNullOrWhiteSpace(opciones.ClientSecret) || string.IsNullOrWhiteSpace(opciones.Code))
            throw new ActualizacionQuiterException("Falta configurar el acceso a Quiter en el servidor.");
        ...
        if (!respuestaToken.IsSuccessStatusCode) throw new ActualizacionQuiterException($"Quiter rechazó la autorización (HTTP {(int)respuestaToken.StatusCode}).");
        ...
        if (string.IsNullOrWhiteSpace(token)) throw new ActualizacionQuiterException("Quiter respondió sin un token de autorización válido.");
        ...
        if (!respuesta.IsSuccessStatusCode) throw new ActualizacionQuiterException($"Quiter rechazó la actualización del contacto (HTTP {(int)respuesta.StatusCode}).");
    }
}
